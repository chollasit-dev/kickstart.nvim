...
local formatters = {
  ...
  -- nginx = { 'nginxfmt' }, -- TODO: Fix not found
  nix = { 'alejandra' },
  python = { 'black', 'isort' },
  sh = { 'shfmt' },
  sql = { 'sqruff' },
...
local formatters_by_ft = vim.tbl_extend('force', general_formatters, formatters)

local linters_by_ft = {
  bash = { 'shellcheck' },
  dockerfile = { 'hadolint' },
  gitcommit = { 'gitlint', 'commitlint' },
  -- go = { 'golangci-lint' }, -- TODO: Fix not available on lint trigger
  haskell = { 'hlint' },
  make = { 'checkmake' },
...
for _, val in pairs(vim.tbl_extend('force', formatters, linters_by_ft)) do
  for _, v in ipairs(val) do
    local exist = false
    for _, f in ipairs(ensure_installed_extends) do
      if v == f then
        exist = true
      end
    end
    if not exist then
      table.insert(ensure_installed_extends, v)
    end
  end
end
...
return {
  {
    ...
    config = function()
      ...
      vim.list_extend(ensure_installed, ensure_installed_extends)
...

  {
    ...
    config = function()
      ...
      vim.api.nvim_create_user_command('FormatDisable', function(args)
        if args.bang then
          -- Disable formatting only for this buffer
